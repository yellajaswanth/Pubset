...
class BucketingDataLoader(object):
    def __init__(self, batch_size, train, bucket=100, shuffle=True):
        if train:
            self.db = json.load(open('dataset/pubmed_small/train_indices.json', 'rb'))
        else:
            self.db = json.load(
                open('dataset/pubmed_small/val_indices.json', 'rb'))
    ...
    def __iter__(self):
        # print(f'Total {len(self.db)} examples')
        lens = [5] * len(self.db)
