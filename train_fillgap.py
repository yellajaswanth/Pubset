...
for epoch in range(args.continue_from, args.num_epochs):
    ...
    for step, batch_index in enumerate(tqdm.tqdm(train_dataloader)):
        ...
        with torch.no_grad():
            batch = torch.stack([data[batch_index[i][0]]['bert'][batch_index[i][1]:(batch_index[i][1]+7)] for i in range(len(batch_index))])
            batch = batch.float()
            batch = (batch - b) / w
        ...
        if (step + 1) % EVAL_STEP == 0:
            ...
            with torch.no_grad():
                for eval_step, eval_batch_index in enumerate(eval_dataloader):
                    eval_batch = torch.stack([data[eval_batch_index[i][0]]['bert'][eval_batch_index[i][1]:(eval_batch_index[i][1] + 7)] for i in range(len(batch_index))])
